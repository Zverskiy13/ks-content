...
-- дефолтные рубрики (сеются один раз, если таблица пустая) — надёжно, в самой схеме
INSERT INTO cp_rubrics(title, hint)
SELECT v.title, v.hint FROM (VALUES
...
WHERE NOT EXISTS (SELECT 1 FROM cp_rubrics);
